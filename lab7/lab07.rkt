  ...
  (cond [(number? exp) exp]
        ...
        [(list? exp)
         (cond
           ...
           [else 
            (cond
                [(procedure? (lookup (car exp) env)) (apply (lookup(car exp) env) (cdr (map mapEval exp)))]
                [(eq? (car(car exp)) 'closure) (apply-closure (car exp) (cdr exp))]
